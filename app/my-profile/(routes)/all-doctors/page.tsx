...
export default function AllDoctors() {
  ...
  useEffect(() => {
    fetch(`http://localhost:4000/api/doctor/all-doctors?page=${page}&limit=6`, {}).then(
      (response) => {
  ...
  const deleteDoctors = async (id: any) => {
    const data = await fetch(
      `http://localhost:4000/api/doctor/delete-doctor/${id}`,
      {
        method: "DELETE",
        headers: {
  ...
  const count = allData?.doctorCount;
  return (
    <div className=" border-[1px] w-[800px] mb-5 p-6">
      <div className="border-r border-b border-l shadow-md p-5">
        <h2 className=" text-3xl font-bold mb-8"> {count} Doctors </h2>
        <div className=" flex items-center justify-between">
          <div className="">
            <h3 className=" mb-5"> Name</h3>
            {users.map((e: any) => (
              <>
                <p className=" mb-6">{e.name}</p>
              </>
            ))}
          </div>
          <div className="">
            <h3 className=" mb-5"> specialization</h3>
            {users.map((e: any) => (
              <>
                <p className=" mb-6">{e.specialization}</p>
              </>
            ))}
          </div>
          <div className="">
              ...
              <>
                <div className=" flex  mb-6" key={e._id}>
                  <PencilIcon
                    ...
                    onClick={() =>
                      router.push(`/my-profile/update-doctor/${e._id}`)
                    }
                  />
                  <Trash
                    fill="#b91c1c"
